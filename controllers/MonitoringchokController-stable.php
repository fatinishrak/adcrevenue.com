...
class MonitoringchokController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
			...
			$recipient_id = \Yii::$app->user->identity->userof;
			if($model->save()) {				
				Notification::notify(Notification::KEY_NEW_MESSAGE, $recipient_id, $model->mamlaNong);	
			}
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
        	if($model->save()) {
				...
				Notification::notify(Notification::KEY_MEETING_REMINDER, $recipient_id, $model->mamlaNong);
	            if ($image !== false) { 
